...
class YNABScraper {
	...
	private mergeItems(existingItems: DataItem[], newItems: DataItem[]): DataItem[] {
		...
		// Create a map of new items by Name + Link
		const newItemsMap = new Map<string, DataItem>();
		newItems.forEach((item) => {
			const key = `${item.Name}||${item.Link}`;
			newItemsMap.set(key, item);
		});

		// Mark existing items as deleted if not found in new items
		const currentDate = new Date().toISOString();
		existingItems.forEach((existingItem) => {
			const key = `${existingItem.Name}||${existingItem.Link}`;
			if (!newItemsMap.has(key) && !existingItem['Date not found']) {
				// Item not found in new items and not already marked as deleted
				const index = merged.indexOf(existingItem);
				if (index !== -1) {
					merged[index] = {
						...existingItem,
						// Preserve "Date not found" if already set
						'Date not found': existingItem['Date not found'] || currentDate
					};
					this.log(`Marked item as deleted: ${existingItem.Name}`);
				}
			}
		});
		...
		newItems.forEach((newItem) => {
			...
			if (existingIndex !== undefined) {
				// Update existing item but preserve original "Date added" and custom fields
	...
	private saveData(items: DataItem[]): void {
		try {
			fs.writeFileSync(this.dataFilePath, JSON.stringify(items, null, 4));
